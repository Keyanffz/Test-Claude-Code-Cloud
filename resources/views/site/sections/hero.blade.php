...
<section class="container-page flex min-h-[80svh] flex-col pt-6 pb-8 md:min-h-[calc(100svh-3.5rem)] md:pt-8" aria-labelledby="hero-title">
    ...
    <div class="grid-page flex-1 content-end gap-y-6 pt-16">
        <h1 id="hero-title" class="col-span-12 font-display {{ $nameSize }}">
            <x-split-text :text="$profile->displayName()" />
        </h1>

        <p class="col-span-12 font-display text-[clamp(1.75rem,3.6vw,3.25rem)] leading-[1.02] tracking-[-0.01em] md:col-span-8 md:col-start-5">
            {{-- The ampersand is set in the serif italic, in the accent color: the one flourish in the hero. --}}
            <x-split-text :text="$profile->headline" accent="&" :delay="0.25" />
        </p>
    </div>

    <div class="grid-page mt-10 gap-y-8 border-t border-line pt-6 md:mt-14">
        @if ($profile->short_bio)
            <p class="col-span-12 max-w-[46ch] text-base leading-relaxed text-muted md:col-span-6 lg:col-span-5 lg:text-lg rise" style="--rise-delay: 0.5s">
                {{ $profile->short_bio }}
            </p>
        @endif

        <div class="col-span-12 flex flex-wrap items-center gap-x-8 gap-y-4 md:col-span-6 md:justify-self-end lg:col-span-5 lg:col-start-8 rise" style="--rise-delay: 0.6s">
